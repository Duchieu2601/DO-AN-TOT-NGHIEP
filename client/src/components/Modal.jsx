...
const Modal = ({
  ...
  const handleActive = (code, value) => {
    ...
    const isDuoi = value.toLowerCase().includes("dưới");
    const isTren = value.toLowerCase().includes("trên");
    if (isDuoi) {
      // "Dưới X" → slider từ 0 đến X
      setPersent1(0);
      setPersent2(convertto100(arrMaxMin[0]));
    } else if (isTren) {
      // "Trên X" → slider kéo hết về cuối
      setPersent1(100);
      setPersent2(100);
    } else {
      // "Từ X - Y" → slider từ X đến Y
  ...
  const handleBeforeSubmit = (e) => {
    ...
    handleSubmit(
      e,
      {
        ...
        [name]: `Từ ${convert100toTarget(persent1)} - ${convert100toTarget(persent2)} ${name === "price" ? "triệu" : "m"}`,
      },
      {
